...
//
// Invocation modes:
// - { org_id }                -> run for one org (manual UI button)
// - { org_id, participant_id } -> run for one org + one participant
// - {}                         -> run for every org with completed visits (cron)
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // No org_id => scheduled run, fan out across every org with completed visits.
    let orgIds: string[] = [];
    if (org_id) {
      orgIds = [org_id];
    } else {
      const { data: orgs } = await sb
        ...
        .select("org_id")
        .eq("status", "completed");
      orgIds = Array.from(new Set((orgs ?? []).map((r: any) => r.org_id))).filter(Boolean);
    }

    const allCreated: any[] = [];
    const allSkipped: any[] = [];
    let consideredTotal = 0;
    for (const currentOrg of orgIds) {
      const r = await runForOrg(sb, currentOrg, participant_id);
      allCreated.push(...r.created);
      allSkipped.push(...r.skipped);
      consideredTotal += r.considered;
    }
...
async function runForOrg(sb: any, org_id: string, participant_id?: string) {
  let q = sb
    .from("visits")
    .select(
  ...
  const geoByVisit: Record<string, any[]> = {};
  const contribByVisit: Record<string, any[]> = {};
  if (visitIds.length) {
  ...
  return { created, skipped: skippedReport, considered: (visits ?? []).length };
}